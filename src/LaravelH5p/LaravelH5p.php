...
/*
 *
 * @Project        thomasmars/laravel-h5p
...
namespace thomasmars\LaravelH5p;
...
use thomasmars\LaravelH5p\Storages\LaravelH5pStorage;
use thomasmars\LaravelH5p\Repositories\LaravelH5pRepository;
use thomasmars\LaravelH5p\Repositories\EditorAjaxRepository;
use thomasmars\LaravelH5p\Storages\EditorStorage;
...
//                $this->enqueue_assets($files);
                foreach ($files['scripts'] as $script) {
                    $url = $script->path . $script->version;
                    if (!in_array($url, $settings['loadedJs'])) {
                        $settings['loadedJs'][] = self::get_h5p_storage($url);
                    }
                }
                foreach ($files['styles'] as $style) {
                    $url = $style->path . $style->version;
                    if (!in_array($url, $settings['loadedCss'])) {
                        $settings['loadedCss'][] = self::get_h5p_storage($url);
                    }
                }
            } elseif ($embed === 'iframe') {
                $settings['contents'][$cid]['scripts'] = $core->getAssetsUrls($files['scripts']);
